...
{
    protected async Task ArrangeStatusAsync(AsyncCompensator compensator, CompensatorStatus status)
    {
        switch (status)
        {
            case CompensatorStatus.FailedToExecute:
            case CompensatorStatus.Compensating:
                typeof(AsyncCompensator)
                    .GetProperty(nameof(AsyncCompensator.Status), BindingFlags.Public | BindingFlags.Instance)?
            ...
            case CompensatorStatus.FailedToCompensate:
                await compensator.AddCompensationAsync(() => throw new InvalidOperationException("Failed to compensate"));
                try
                {
                    await compensator.CompensateAsync();
                }
    ...
    protected async Task<object[]> ArrangeTagsAndCompensationsAsync(AsyncCompensator compensator)
    {
        var foreachTag = await compensator.CreateTagAsync("foreachTag");

        var doTag = await compensator.CreateTagAsync("doTag");

        var doHelper1 = new DoHelper("doHelper1");
        await compensator.DoAsync(doHelper1.ExecuteAsync, doHelper1.CompensateAsync, doTag);

        var doHelper2 = new DoHelper("doHelper2");
        await compensator.DoAsync(doHelper2.ExecuteAsync, doHelper2.CompensateAsync, doTag);

        var doHelper3 = new DoHelper("doHelper3");
        await compensator.DoAsync(doHelper3.ExecuteAsync, doHelper3.CompensateAsync);

        var doIfTag = await compensator.CreateTagAsync("doIfTag");

        var doIfHelper1 = new DoIfHelper(TestOptions.ShouldBeCalledAndReturnTrue, "doIfHelper1");
        await compensator.DoIfAsync(doIfHelper1.TestAsync, doIfHelper1.ExecuteAsync, doIfHelper1.CompensateAsync);

        var doIfHelper2 = new DoIfHelper(TestOptions.ShouldBeCalledAndReturnTrue, "doIfHelper2");
        await compensator.DoIfAsync(doIfHelper2.TestAsync, doIfHelper2.ExecuteAsync, doIfHelper2.CompensateAsync);

        var doIfHelper3 = new DoIfHelper(TestOptions.ShouldBeCalledAndReturnFalse, "doIfHelper3");
        await compensator.DoIfAsync(doIfHelper3.TestAsync, doIfHelper3.ExecuteAsync, doIfHelper3.CompensateAsync, doIfTag);

        var doIfHelper4 = new DoIfHelper(TestOptions.ShouldBeCalledAndReturnTrue, "doIfHelper4");
        await compensator.DoIfAsync(doIfHelper4.TestAsync, doIfHelper4.ExecuteAsync, doIfHelper4.CompensateAsync, doIfTag);

        var getTag1 = await compensator.CreateTagAsync("getTag1");
        var getTag2 = await compensator.CreateTagAsync("getTag2");
        var getTag3 = await compensator.CreateTagAsync("getTag3");

        var getHelper1 = new GetHelper("getHelper1");
        await compensator.GetAsync(getHelper1.ExecuteAsync, getHelper1.CompensateAsync);

        var getHelper2 = new GetHelper("getHelper2");
        await compensator.GetAsync(getHelper2.ExecuteAsync, getHelper2.CompensateAsync, getTag2);

        var getHelper3 = new GetHelper("getHelper3");
        await compensator.GetAsync(getHelper3.ExecuteAsync, getHelper3.CompensateAsync, getTag1);
        ...
        await compensator.ForeachAsync(foreachHelper1.Items, foreachHelper1.ExecuteAsync, foreachHelper1.CompensateAsync);
        ...
        await compensator.ForeachAsync(foreachHelper2.Items, foreachHelper2.ExecuteAsync, foreachHelper2.CompensateAsync, foreachTag);
        ...
        await AssertInternalCompensationOrderAsync(compensator, expectedOrderedHelpers.OfType<HelperBase>().ToArray());
    ...
    protected async Task AssertInternalCompensationOrderAsync(AsyncCompensator compensator, params HelperBase[] expectedOrderedItems)
    {
        // make sure not null
        ...
        for (var i = 0; i < expectedOrderedItems.Length; i++)
        {
            ...
            Assert.True(await expectedItem.IsExpectedCompensationAsync(actualCompensation));
        }
    }
}